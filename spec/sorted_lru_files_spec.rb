...
RSpec.describe(SortedLruFiles) do
  ...
  it 'can read back an inserted item' do
    sorted_files.insert(expected1, "https://mslinn.com/#{expected1}") # insert reverses the string
  ...
  it 'handles multiple matches' do
    ...
    expect(actual.first&.start_with?(expected.reverse)).to be true
  end
  ...
  it 'works with 3 items' do
    ...
    actual = sorted_files.select expected1
    expect(actual.length).to eq(1)
    expect(actual.first&.url&.start_with?(expected1)).to be true

    actual = sorted_files.select expected2
    expect(actual.length).to eq(1)
    expect(actual.first&.url&.start_with?(expected2)).to be true

    actual = sorted_files.select expected3
    expect(actual.length).to eq(2)
    expect(actual.first&.url&.start_with?(expected3)).to be true

    expected = '.html'
    actual = sorted_files.select expected
    expect(actual.length).to eq(3)
    expect(actual.first&.url&.start_with?(expected)).to be true
  end
end
